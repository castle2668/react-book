...
React 是透過操作 DOM 去完成 UI 渲染，每當元件的 State 改變，React 就會重新渲染那個元件，讓畫面產生改變。但是有些東西是 React 無法 Handle 的，因為這些東西不是 React 負責的，所以會產生一些 Side Effect，如果沒處理好的話，可能會發生像是重複渲染等問題使得網頁效能變差。

哪些是 (Out)side effects 呢？
...
因此，如果今天遇到這些 Effects，我們就會需要透過 `useEffect()` 來處理哩。
...
- Dependency array 是空陣列：只在初始渲染 Component 時執行一次
  - 就只會 Run 那麼一次，之後其他 State 更動都不會再執行這裡的行為哩
- Dependency array 裡面有某些值：當這些值更改時，就會觸發 `useEffect`
  - 初始會跑一次，之後會監聽 State 的改變，如果有更改就會執行
