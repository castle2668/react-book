...
## Get Started: The BrowserRouter Component
...
`<Switch>` component：把它包在 `<Route>` 的外層，React Router 會由上而下尋找與網址相應的路由，一旦找到就會停止配對（沒有使用時，會檢查所有路由是否符合，並不會由上而下檢查），確保同一時間只會有一個路由被加載！

搭配 `<Route>` 的 `exact` prop，讓路由只有在完全匹配到的時候才會生效。

加上 `exact` 前，網址為 `our-domain.com/products/book` 時，會進入率先找到的 `<Products>`，而非完全對應的動態路由 `<ProductDetail>`。

加上 `exact` 後，就能正確匹配到相對應的路由。
